...
resource "aws_instance" "jenkins-worker-oregon" {
  ...
  provisioner "remote-exec" {
    when = destroy
    inline = [
      "java -jar /home/ec2-user/jenkins-cli.jar -auth @/home/ec2-user/jenkins_auth -s http://${aws_instance.jenkins-master.private_ip}:8080 -auth @/home/ec2-user/jenkins_auth delete-node ${self.triggers.private_ip}"
    ]
    connection {
      type        = "ssh"
      user        = "ec2-user"
